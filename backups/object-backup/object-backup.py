...
print(f"Backing up from {config['local']['bucket']} at {config['local']['endpoint_url']} to {config['archive']['bucket']} at {config['archive']['endpoint_url']}")
...
try:
    _ = [obj.key for obj in archive_bucket.objects.limit(1)] # Check that can read from bucket
except:
...
print("Check that some files already match as a way of checking that the correct buckets are being/prefix used.")
keys = []
i = 0
...
if matching < 50:
    print(f"{matching} out of 100 objects are already on the target bucket. Canceling backup.")
    time.sleep(2)
    sys.exit(0)
print("Done.")
...
def handle_file(obj):
    ...
    try:
        ...
        if obj.e_tag == archive_obj.e_tag:
            modified = False
        elif ('-' in obj.e_tag or '-' in archive_obj.e_tag) and obj.size == archive_obj.content_length: #archive_object is a s3.Object so use content_length to get the size  

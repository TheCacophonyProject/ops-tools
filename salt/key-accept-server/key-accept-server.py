...
def check_key_status(minion_id):
    result = subprocess.run(
        ["sudo", "salt-key", "--list=all", "--out=json"], capture_output=True, text=True
    )
...
def accept_key(minion_id):
    result = subprocess.run(
        ["sudo", "salt-key", "--accept", minion_id, "-y"],
        capture_output=True,
        text=True,
    )
...
def accept_salt_key():
    ...
    if not re.match(r"^tc2-\d{4}$", minion_id):
        return (
            jsonify(
                {
                    "status": "error",
                    "message": "Minion ID must be in the format tc2-1234",
                }
            ),
            400,
        )
    ...
    if key_status == "accepted":
        return jsonify(
            {
                "status": "success",
                "message": f"Key for minion {minion_id} is already accepted",
            }
        )
    elif key_status == "unaccepted":
        if accept_key(minion_id):
            return jsonify(
                {"status": "success", "message": f"Key for minion {minion_id} accepted"}
            )
        else:
        ...
        return (
            jsonify({"status": "error", "message": "Failed to check key status"}),
            500,
        )
